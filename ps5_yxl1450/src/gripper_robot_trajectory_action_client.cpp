...
int main(int argc, char** argv) {
	...
	safe_jnts[1] = 10.0/180.0*M_PI; // joint2, a little bit forward
	safe_jnts[2] = 80/180.0*M_PI; // joint3, a little bit forward
	...
	ROS_INFO("Beer pose: x: %f, y:%f, z:%f",beer_pos.x,beer_pos.y,beer_pos.z);
	...
	hover_open_start_pos.z += beer_height/2 + lift_height;
	std::vector<double> hover_open_start_jnts; // the corresponding joint position
	hover_open_start_jnts.resize(6);
	hover_open_start_jnts = calcInvK(hover_open_start_pos, gripper_open); // calculate the robot joints
	...
	around_open_start_jnts = calcInvK(around_open_start_pos, gripper_open);
	...
	grasp_close_start_jnts = calcInvK(grasp_close_start_pos, gripper_close);
	...
	ros::Duration(30).sleep(); // delay before jumping to next task
	...
	hover_close_start_pos.z = hover_close_start_pos.z + lift_height+.5;
	std::vector<double> hover_close_start_jnts;
	hover_close_start_jnts.resize(6);
	hover_close_start_jnts = calcInvK(hover_close_start_pos, gripper_close);
	...
	hover_close_end_jnts = calcInvK(hover_close_end_pos, gripper_close);
	...
	ros::Duration(5).sleep(); // delay before jumping to next task
	...
	grasp_close_end_jnts = calcInvK(grasp_close_end_pos, gripper_close);
	...
	around_open_end_jnts = calcInvK(around_open_end_pos, gripper_open);
	...
	hover_open_end_jnts = calcInvK(hover_open_end_pos, gripper_open);
