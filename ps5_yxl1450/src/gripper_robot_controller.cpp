...
private:
	...
	bool kpkvCallback(ps5_yxl1450::kpkv_msgRequest& request,
			ps5_yxl1450::kpkv_msgResponse& response);
	// service clients
...
Joint::Joint(ros::NodeHandle nh, std::string joint_name, double dt) {
	...
	pos_cmd_subscriber = nh.subscribe(joint_name + "_pos_cmd", 1,
			&Joint::posCmdCB, this);
	// initialize kpkv service server
	kpkv_server = nh.advertiseService(joint_name + "_kpkv_service",
			&Joint::kpkvCallback, this);
...
bool Joint::kpkvCallback(ps5_yxl1450::kpkv_msgRequest& request,
		ps5_yxl1450::kpkv_msgResponse& response) {
	ROS_INFO("kpkvCallback activated");
	kp = request.kp;
	kv = request.kv;
	// joint_name has to be converted to c_str() for ROS_INFO output
	ROS_INFO("%s: kp has been set to %f, kv has been set to %f",
			joint_name.c_str(), kp, kv);
...
int main(int argc, char **argv) {
	...
	double dt = 0.001; // sample time for the controller
	...
	// with gravity -0.1
	joint1.kpkvSetting(60, 15);
	joint2.kpkvSetting(1000, 300);
	joint3.kpkvSetting(500, 150);
	joint4.kpkvSetting(30, 9);
	joint5.kpkvSetting(60, 18);
	joint6.kpkvSetting(60, 18);
