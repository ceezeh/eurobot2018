...
inline bool read_landmark_data(std::string filename,
		std::vector<BeaconObs>& observations) {
...
public:
	...
	void calRotYXZ(float pitch, float yaw, float roll, float Ys, float Zs,
			Mat &R);
	void getAnglesYXZ(float &pitch, float &yaw, float &roll, float &Ys,
			float &Zs, Mat R);

	void get6DOFMarkerPose(TrackerMultiMarker* tracker, Mat cam_pose,
			Mat &marker_pose, int index);
